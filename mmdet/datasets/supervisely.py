...
class SuperviselyDataset(CustomDataset):
    ...
    def _load_ann(self,imgpath):
        #  project.datasets.get('ds1').get_item_paths('pexels-photo-708392.png')
        item_paths = self.project.datasets.get(imgpath.split('/')[0]).get_item_paths(imgpath.strip().split('/')[-1])
